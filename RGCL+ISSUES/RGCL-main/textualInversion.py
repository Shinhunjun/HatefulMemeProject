...
class LinearProjection(nn.Module):
    def __init__(self, input_dim, output_dim, num_layers, drop_probs):
        ...
        for _ in range(1, num_layers):
            map_layers.extend([nn.ReLU(), nn.Linear(output_dim, output_dim), nn.Dropout(p=drop_probs[0])])
...
class TextualInversion(nn.Module):
    def __init__(self, clip_model: CLIP, clip_img_enc_output_dim: int, phi_proj: bool, text_proj: bool, post_proj: bool, drop_probs, phi_freeze: bool, enh_text: bool, post_dim=None, num_pre_proj_layers=1):
        ...
        assert self.clip_model.token_embedding.embedding_dim == PHI_INPUT_DIM, 'CLIP model selected is not compatible with the pre-trained phi network'
        ...
        if phi_proj:
            ...
            phi_map_layers = [nn.Linear(PHI_INPUT_DIM, PHI_INPUT_DIM), nn.Dropout(p=drop_probs[0])]
            self.phi_map = nn.Sequential(*phi_map_layers)

        # load pre-trained weights of phi
        phi_dict = torch.load("./data/CLIP_Embedding/HarMeme/phi_imagenet_45.pt")["MLPCustom"]
        with torch.no_grad():
        ...
        pre_inversion_layers = [nn.Linear(in_dim, PHI_INPUT_DIM), nn.Dropout(p=drop_probs[0])]
        ...
        if post_proj:
            ...
            post_inversion_layers = [nn.Linear(self.clip_model.token_embedding.embedding_dim, post_dim), nn.Dropout(p=drop_probs[0])]
            self.post_inversion_map = nn.Sequential(*post_inversion_layers)

    def encode_with_vstar(self, clip_model: CLIP, text: torch.tensor, v_star: torch.tensor, num_vstar=1, pooling=True, token_id=259, proj=True):
        ...
        if v_star.shape[0] == x.shape[0]:
            ...
            x[torch.arange(x.shape[0]).repeat_interleave(num_vstar).reshape(x.shape[0], num_vstar), rep_idx.T] = v_star.to(x.dtype)
        else:
            print(f'v_star.shape[0]: {v_star.shape[0]} and x.shape[0]: {x.shape[0]}')
            raise ValueError()
