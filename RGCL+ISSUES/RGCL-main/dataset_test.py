...
from feature_loader import get_attrobj_from_ids
...
def get_values_from_gt_test(dataset, split):
    """
    Extract image path, text, gt_label and image ids from gt files
    input: dataset name, split name
    output: list of ... for input to image_text_dataset
    ...
    if dataset == "HarMeme":
        ...
        list_label = list_label_converted
        for img_id in list_ids:
            list_image_path.append("./data/image/" + dataset + "/Test/" + img_id + ".png")
    elif dataset == "HarmC" :
        # The HarmC is the same as HarMeme, but here we refer to 3 classes
        # 0: not harmful, 1: 
        list_label = gt_df["labels"]
...
def get_Dataloader(
    ...
    imgtxt_dataset = image_text_dataset(
        get_values_from_gt_test(dataset, "test_new"),
        preprocess,
        image_size=image_size,
    )
    test_seen = DataLoader(
        imgtxt_dataset, batch_size=1, num_workers=num_workers
...
def load_feats_HarMeme(path, model):
    dataset = "HarMeme"
    train = load_feats_split("{}/{}/train_{}.pt".format(path, dataset, model))
    dev = load_feats_split("{}/{}/val_{}.pt".format(path, dataset, model))
    test_seen = load_feats_split("{}/{}/test_new_{}_test.pt".format(path, dataset, model))
    return train, dev, test_seen
